...
def generate_demo_data(seed=42):
    ...
    return DashboardData(
        ...
        metadata={"source": "demo", "message": "Running on demo data. Swap in real outputs when ready."},
    )
...
def load_real_data():
    ...
    return DashboardData(predictions, force_month, lsoa_totals, model_summary, funding, deprivation, kde_points, {"source": "real", "message": "Using model outputs from outputs/tables."})
...
def filter_by_sidebar(data):
    ...
    with st.sidebar:
        ...
        if data.metadata["source"] == "demo":
            ...
            st.success("Using model output files")
...
def render_status_legend():
    st.markdown(
        ...
        <span class="status-pill pill-low">High</span> Stable across validation checks.<br>
        <span class="status-pill pill-moderate">Moderate</span> Usable with some caveats.<br>
        <span class="status-pill pill-caution">Use with caution</span> Don't rely on this alone.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def page_overview(data, forces, crimes, months, estimate_mode):
    ...
    st.markdown('<div class="section-note">Top forces by estimated demand, plus funding alignment.</div>', unsafe_allow_html=True)
    ...
    with c1: kpi("Observed demand", format_number(total_observed), "Recorded over the selected period")
    with c2: kpi("Estimated demand", format_number(total_estimated), "Based on selected estimate type")
    with c3: kpi("Highest-pressure force", highest_force, "Top by estimated demand")
    with c4: kpi("Caution flags", format_number(caution_count), "Low-confidence estimates")
    ...
    with right:
        ...
        if not funding.empty:
            ...
            st.info("Funding data not loaded.")
...
def page_explore_force(data, forces, crimes, months, estimate_mode):
    estimate_col = select_estimate_column(estimate_mode)
    selected_force = st.selectbox("Pick a force", forces or POLICE_FORCES)
    ...
    st.header(f"Explore: {selected_force}")
    ...
    with left:
        if not fm.empty:
            ...
            fig = px.pie(crime_comp, names="crime_type", values=estimate_col, title="Crime-type contribution", hole=0.42)
            st.plotly_chart(theme_plotly(fig), use_container_width=True)
    with right:
        if not lsoa.empty:
    ...
    with st.expander("Local-area table"):
...
def page_hotspot_map(data, forces, crimes, months, estimate_mode):
    ...
    st.markdown('<div class="section-note">Spatial demand view. Uses demo points until the KDE outputs are wired in.</div>', unsafe_allow_html=True)
    ...
    if points.empty:
        st.info("No hotspot points for the current filters.")
        return
    ...
    if mode == "KDE-style heatmap":
        fig = px.density_mapbox(
            ...
            title="KDE-style demand intensity",
        )
    else:
        if lsoa.empty or "lat" not in lsoa.columns or "lon" not in lsoa.columns:
            st.info("Local-area coordinates not available.")
            return
        value_col = "recommended_prediction" if "recommended_prediction" in lsoa.columns else "predicted_count"
...
def page_demand_context(data, forces, crimes, months, estimate_mode):
    ...
    st.markdown('<div class="section-note">Placeholder for IMD/deprivation and other contextual layers.</div>', unsafe_allow_html=True)

    st.subheader("Deprivation context")
    if data.deprivation.empty:
        st.info("IMD data not loaded.")
    else:
        ...
        fig = px.line(
            ...
            y="predicted_demand",
            markers=True,
            title="Estimated demand by deprivation decile (placeholder)",
            labels={"imd_decile": "IMD decile (1 = most deprived)", "predicted_demand": "Estimated demand"},
            color_discrete_sequence=["#ff6a3d"],
        )
        st.plotly_chart(theme_plotly(fig), use_container_width=True)
        st.caption("Descriptive only - not causal evidence.")
...
def page_method_notes(data):
    ...
    st.write("Estimated police demand from recorded crime data. Modelled at LSOA-month level and aggregated to police force for comparison.")

    st.subheader("Reading the estimates")
    st.markdown(
        """
        - **Predicted demand** - expected recorded crime demand for the chosen force, crime type, and period.
        - **Expected range** - uncertainty around the estimate.
        - **Demand pressure** - relative ranking across forces or local areas.
        - **Use with caution** - estimate needs supporting evidence before being acted on.
        """
    )

    st.subheader("Prototype caveats")
    st.markdown(
        """
        - Data may be demo placeholder until model outputs are frozen.
        - Adjacency uses centroid-distance kNN, not true polygon boundaries.
        - Population offset was tested but isn't the primary estimate.
        - IMD and KDE layers are planned but not yet integrated.
        - COVID handling still needs to be agreed before final modelling.
        """
    )

    with st.expander("Technical details"):
        st.markdown("Bayesian Negative Binomial count model with LSOA effects, month effects, time trend, and a centroid-distance spatial-lag feature.")
...
def main():
    st.markdown(
        ...
        <div class="hero-box">
            ...
            <div class="hero-text">
            Prototype for exploring estimated police demand across territorial forces in England and Wales.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    try:
        ...
        st.error("Couldn't load model output files. Falling back to demo data.")
        st.exception(exc)
